...
const (
	...
	// chReadyTable appears in SHOW TABLES once the init SQL has finished.
	...
	// chUsersMount overrides the image's loopback-only default user, which
	// would 403 the agg/api connecting from other containers.
...
// startClickHouse starts a single-node ClickHouse with the committed config and
// the v6 schema mounted in, and waits for readiness.
func startClickHouse(ctx context.Context, rt Runtime, container, network, repoRoot, artifactsDir string) (*clickHouse, error) {
	...
	if err == nil {
		...
		// Readiness is probed via Exec; the caller fails on a missing IP.
...
// waitClickHouseReady polls until SELECT 1 answers and SHOW TABLES includes
// chReadyTable.
...
// tables returns the tables in the default DB, for the summary.
...
// v6InitSQL is the localdebug v6 cluster schema de-clustered for a single node
// (no Keeper), plus the aggregator's internal-log tables so its inserts do not
// log a 404 every few seconds.
