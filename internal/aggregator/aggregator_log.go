...
func (a *Aggregator) appendInternalLogLocked(typ string, key0 string, key1 string, key2 string, key3 string, key4 string, key5 string, message string) {
	if a.config.StorageBackend == duckstore.BackendDuck { // no ClickHouse log table to insert into
		log.Printf("[internal_log] type=%s keys=[%s %s %s %s %s %s] %s", typ, key0, key1, key2, key3, key4, key5, message)
		return
	}
