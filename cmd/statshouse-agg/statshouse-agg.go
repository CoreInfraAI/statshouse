...
func parseCommandLine() error {
	...
	flag.Var(&argv.StorageBackend, "storage-backend", "where metric data is stored: \"clickhouse\" (default) or \"duck\" (DuckDB embedded in the aggregator; needs a binary built with the duckdb build tag, see docs/duck-store.md)")
	flag.StringVar(&argv.DuckStoreDir, "duck-store-dir", aggregator.DefaultConfigAggregator().DuckStoreDir, "duck-store: directory holding the shard's DuckDB file; required with --storage-backend=duck")
	flag.DurationVar(&argv.DuckRetention1s, "duck-retention-1s", aggregator.DefaultConfigAggregator().DuckRetention1s, "duck-store: how long 1s-tier rows are kept; 0 keeps them forever")
	flag.DurationVar(&argv.DuckRetention1m, "duck-retention-1m", aggregator.DefaultConfigAggregator().DuckRetention1m, "duck-store: how long 1m-tier rows are kept; 0 keeps them forever")
	flag.DurationVar(&argv.DuckRetention1h, "duck-retention-1h", aggregator.DefaultConfigAggregator().DuckRetention1h, "duck-store: how long 1h-tier rows are kept; 0 keeps them forever")
	flag.IntVar(&argv.DuckQueryConcurrency, "duck-query-concurrency", aggregator.DefaultConfigAggregator().DuckQueryConcurrency, "duck-store: how many store queries execute at once; others wait for a slot until their deadline")
	flag.Int64Var(&argv.DuckMemoryLimit, "duck-memory-limit", aggregator.DefaultConfigAggregator().DuckMemoryLimit, "duck-store: DuckDB memory limit in bytes")
